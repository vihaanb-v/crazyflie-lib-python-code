...
pub trait ExprFormatter {
    ...
    fn function_call(&self, function: Function, args: Vec<Expr>, ty: &Type) -> Self::Return;
    ...
    fn expr(&self, expr: Expr) -> Self::Return {
        match expr.kind {
            ...
            ExprKind::FunctionCall(f, args) => self.function_call(f, args, &expr.ty),
        ...
        + ConstantFormatter<Return = String>,
{
    ...
    /// Returns the code, that is used to call a function.
    fn function_call(&self, function: Function, args: Vec<Expr>, ty: &Type) -> String {
        let arg_tys = args.iter().map(|e| e.ty.clone()).collect::<Vec<_>>();
        let args = args.into_iter().map(|e| self.expr(e)).join(", ");
        format!("{}({})", self.function(function, &arg_tys, ty), args)
    }
}
    ...
    X: DefaultExprFormatter,
{
    ...
    fn function_call(&self, function: Function, args: Vec<Expr>, ty: &Type) -> Self::Return {
        <Self as DefaultExprFormatter>::function_call(self, function, args, ty)
    }
}
...
pub trait FunctionFormatter {
    ...
    fn function_sqrt(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the absolute value function in the target language.
    fn function_abs(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the sin function in the target language.
    fn function_sin(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the arcsin function in the target language.
    fn function_arcsin(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the cos function in the target language.
    fn function_cos(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the arccos function in the target language.
    fn function_arccos(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the tan function in the target language.
    fn function_tan(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the arctan function in the target language.
    fn function_arctan(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the min function in the target language.
    fn function_min(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Returns the name of the max function in the target language.
    fn function_max(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return;

    /// Formats a function in the target language.
    fn function(&self, f: Function, arg_ty: &[Type], return_ty: &Type) -> Self::Return {
        match f {
            Function::Sqrt => self.function_sqrt(arg_ty, return_ty),
            Function::Abs => self.function_abs(arg_ty, return_ty),
            Function::Sin => self.function_sin(arg_ty, return_ty),
            Function::Arcsin => self.function_arcsin(arg_ty, return_ty),
            Function::Cos => self.function_cos(arg_ty, return_ty),
            Function::Arccos => self.function_arccos(arg_ty, return_ty),
            Function::Tan => self.function_tan(arg_ty, return_ty),
            Function::Arctan => self.function_arctan(arg_ty, return_ty),
            Function::Min => self.function_min(arg_ty, return_ty),
            Function::Max => self.function_max(arg_ty, return_ty),
        }
    }
}
...
pub trait DefaultFunctionFormatter {
    /// Returns the name of the squareroot function in the target language.
    fn function_sqrt(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "sqrt".into()
    }

    /// Returns the name of the absolute value function in the target language.
    fn function_abs(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "abs".into()
    }

    /// Returns the name of the sin function in the target language.
    fn function_sin(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "sin".into()
    }

    /// Returns the name of the arcsin function in the target language.
    fn function_arcsin(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "arcsin".into()
    }

    /// Returns the name of the cos function in the target language.
    fn function_cos(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "cos".into()
    }

    /// Returns the name of the arccos function in the target language.
    fn function_arccos(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "arccos".into()
    }

    /// Returns the name of the tan function in the target language.
    fn function_tan(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "tan".into()
    }

    /// Returns the name of the arctan function in the target language.
    fn function_arctan(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "arctan".into()
    }

    /// Returns the name of the min function in the target language.
    fn function_min(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "min".into()
    }

    /// Returns the name of the max function in the target language.
    fn function_max(&self, _arg_ty: &[Type], _return_ty: &Type) -> String {
        "max".into()
    }
}
    ...
    X: DefaultFunctionFormatter,
{
    ...
    fn function_sqrt(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_sqrt(self, arg_ty, areturn_ty)
    }

    fn function_abs(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_abs(self, arg_ty, areturn_ty)
    }

    fn function_sin(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_sin(self, arg_ty, areturn_ty)
    }

    fn function_arcsin(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_arcsin(self, arg_ty, areturn_ty)
    }

    fn function_cos(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_cos(self, arg_ty, areturn_ty)
    }

    fn function_arccos(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_arccos(self, arg_ty, areturn_ty)
    }

    fn function_tan(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_tan(self, arg_ty, areturn_ty)
    }

    fn function_arctan(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_arctan(self, arg_ty, areturn_ty)
    }

    fn function_min(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_min(self, arg_ty, areturn_ty)
    }

    fn function_max(&self, arg_ty: &[Type], areturn_ty: &Type) -> Self::Return {
        <Self as DefaultFunctionFormatter>::function_max(self, arg_ty, areturn_ty)
    }
}
